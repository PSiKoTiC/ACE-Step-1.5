...
class LyricScoreMixin(LyricAlignmentCommonMixin):
    ...
    def get_lyric_score(
        ...
        try:
            ...
            # Explicitly free all large intermediate CUDA tensors.
            # The decoder forward pass with output_attentions=True generates
            # attention matrices for every layer — these are the primary cause
            # of VRAM accumulation across successive score calls.
            del decoder_outputs, captured_layers, stacked
            del all_layers_matrix_lm, all_layers_matrix_dit
            del pure_matrix_lm, pure_matrix_dit
            del xt_in, xt_lm, xt_dit, x0
            del t_in, encoder_hidden_states_in, encoder_attention_mask_in
            del context_latents_in, attention_mask_in
            gc.collect()
