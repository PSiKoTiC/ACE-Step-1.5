...
class ModelRunner:
    ...
    def exit(self):
        ...
        # Release model weights, KV cache, and sampler from GPU
        ...
        if hasattr(self, "sampler"):
            del self.sampler
        # Release pre-allocated sample buffers (pinned CPU memory)
        for attr in list(vars(self)):
            if attr.startswith("_cpu_") or attr.startswith("_seq_"):
                try:
                    delattr(self, attr)
                except Exception:
                    pass
