...
def _compile_default(tier_default: bool) -> bool:
    """Return the compile-model default, respecting platform constraints.

    On macOS torch.compile is unsupported; on Windows it requires Triton.
    If Triton becomes available (e.g. via a future dependency update), this
    will automatically detect it and allow the tier default through.
    """
    ...
    if sys.platform == "win32":
        try:
            ...
            from triton.compiler import compiler as _tc  # noqa: F401
            return tier_default
        except Exception:
            return False
    return tier_default
...
def on_tier_change(selected_tier, llm_handler=None):
    ...
    return (
        ...
        gr.update(value=_compile_default(new_config.compile_model_default)),
        gr.update(
