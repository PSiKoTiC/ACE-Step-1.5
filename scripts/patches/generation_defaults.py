...
def compute_init_defaults(
    ...
    if sys.platform == "darwin":
        ...
        # torch.compile needs a working Triton backend.  If Triton is
        # importable AND exposes a CUDA target we can assume it works;
        # otherwise default compile to OFF so users don't hit repeated
        # failed compilation attempts that slow inference dramatically.
        try:
            import triton  # noqa: F401
            # Triton exists — check it actually has a CUDA backend
