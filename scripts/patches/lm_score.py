...
def _load_scoring_model_context(llm_handler):
    ...
    # The HF scoring model is always kept on CPU between forward passes
    # (see get_hf_model_for_scoring in llm_inference.py).  Here we move it
    # to the accelerator only for the duration of the scoring forward pass
    # and immediately return it to CPU afterwards, regardless of the
    # offload_to_cpu flag.  This prevents it from permanently occupying
    # VRAM on top of the already-loaded DiT + LM + VAE + text_encoder.
    import gc as _gc
    ...
    device = llm_handler.device if hasattr(llm_handler, "device") else "cuda"

    if hasattr(model, "to"):
        logger.info(f"[scoring] Moving HF scoring model to {device} for forward pass")
        model.to(device)
    ...
    try:
        ...
        if hasattr(model, "to"):
            logger.info("[scoring] Returning HF scoring model to CPU")
            model.to("cpu")
            _gc.collect()
